...
export async function startChannelServer(): Promise<void> {
  ...
  function handleHubMessage(msg: HubMessage): void {
    switch (msg.kind) {
        ...
        if (state.pendingLockResolve) {
          if (msg.success) {
            ...
            state.pendingLockResolve(
              `LOCK DENIED: "${msg.filePath}" is locked${holder}.${msg.reason ? ` ${msg.reason}` : ''} Do NOT edit this file — coordinate with the lock holder first.`,
            );
          }
      ...
      case 'lock_update': {
        const eventLabels = { acquired: 'LOCKED', released: 'UNLOCKED', expired: 'LOCK EXPIRED' };
        const advice = msg.event === 'acquired' ? ' — do NOT edit this file' : ' — file is now available for editing';
        mcp.notification({
          method: 'notifications/claude/channel',
          params: {
            content: `${eventLabels[msg.event]}: "${msg.lock.filePath}" by ${msg.lock.lockedBy}${msg.lock.taskId ? ` (task ${msg.lock.taskId})` : ''}${advice}`,
            meta: { type: 'status', event: `lock_${msg.event}`, peer: msg.lock.lockedBy },
          },
        });
        break;
      }
      ...
      case 'locks_list':
        if (state.pendingLockResolve) {
          if (msg.locks.length === 0) {
            ...
            const now = Date.now();
            const lines = msg.locks.map((l) => {
              const ttl = Math.max(0, Math.round((l.expiresAt - now) / 1000));
              return `  ${l.filePath} — locked by ${l.lockedBy} (expires in ${ttl}s)${l.taskId ? ` [task: ${l.taskId}]` : ''}`;
            });
